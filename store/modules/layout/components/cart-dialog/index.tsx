...
const CartPopover = ({
  cart,
  ...props
}: { cart: HttpTypes.StoreCart | null } & React.ComponentProps<"button">) => {
  ...
  if (!itemSorted || itemSorted.length === 0) return null;
  return (
    <Sheet>
      ...
      >
        ...
        <div className="overflow-y-scroll px-4  max-h-[450px]">
          {itemSorted.map((item) => (
            <li
              key={item.id}
